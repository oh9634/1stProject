...
<jsp:include page="../main/header.jsp" />
	<div id="content_form">
		<table>
			<tr>
			<c:forEach var="dto" items="${searchLectures }">			
				<td>
					<p id="lName">
						${dto.lName }<br>
	...
	<br>
		<div class="paging">
		<a href="${conPath }/lectureList2.do?pageNum=1&schWord=${schWord}">&lt;&lt;</a>
		&nbsp; &nbsp; &nbsp;
		<c:if test="${startPage>BLOCKSIZE }">
			<a href="${conPath }/lectureList2.do?pageNum=${startPage-1}&schWord=${schWord}">&lt;</a>
		</c:if>
		<c:if test="${startPage<=BLOCKSIZE }">
			&lt;
		</c:if>
		...
		<c:forEach var="i" begin="${startPage }" end="${endPage }">
			...
			<c:if test="${i != pageNum }">
				[ <a href="${conPath }/lectureList2.do?pageNum=${i}&schWord=${schWord}">${i }</a> ]
			</c:if>
		</c:forEach>
		&nbsp; &nbsp; &nbsp;
		<c:if test="${endPage < pageCnt }">
			<a href="${conPath }/lectureList2.do?pageNum=${endPage+1}&schWord=${schWord}">&gt;</a>
		</c:if>
		<c:if test="${endPage == pageCnt }">
			&gt;
		</c:if>
		&nbsp; &nbsp; &nbsp;
		<a href="${conPath }/lectureList2.do?pageNum=${pageCnt}&schWord=${schWord}">&gt;&gt;</a>
	</div>
	</div>
