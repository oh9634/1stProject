...
<jsp:include page="../main/header.jsp" />
	<div id="content">
		<table>
		<caption>수강중인 강의목록 <span id="totCnt">| 총 ${totCnt }개</span></caption>
		<c:if test="${totCnt!=0 }">
		<tr><td>
		...
		<c:if test="${totCnt!=0 }">
			<c:forEach items="${orderNowList }" var="dto">
				<tr>
	...
	<div class="paging">
		<c:if test="${startPage > BLOCKSIZE }">
			[ <a href="${conPath }/orderNowList.do?pageNum=${startPage-1}&mId=${mId}"> 이전 </a> ]
		</c:if>
		<c:forEach var="i" begin="${startPage }" end="${endPage }">
			...
			<c:if test="${i != pageNum }">
				[ <a href="${conPath }/orderNowList.do?pageNum=${i}&mId=${mId}"> ${i } </a> ]
			</c:if>
		</c:forEach>
		<c:if test="${endPage<pageCnt }">
		  [ <a href="${conPath }/orderNowList.do?pageNum=${endPage+1}&mId=${mId}"> 다음 </a> ]
		</c:if>
	</div>
	</div>
