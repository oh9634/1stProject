...
<jsp:include page="../main/header.jsp" />
	<div id="content">
		<table>
		<caption>지난 강의목록 <span id="totCnt">| 총 ${totCnt }개</span></caption>
		<c:if test="${totCnt!=0 }">
		<tr><td>
			<input type="button" value="전체목록 " onclick="location='${conPath }/orderList.do?mId=${mId}'" class="lecture_btn">&nbsp;
		...
		<c:if test="${totCnt!=0 }">
			<c:forEach items="${orderPastList }" var="dto">
				<tr>
	...
	<div class="paging">
		<c:if test="${startPage > BLOCKSIZE }">
			[ <a href="${conPath }/orderPastList.do?pageNum=${startPage-1}&mId=${mId}"> 이전 </a> ]
		</c:if>
		<c:forEach var="i" begin="${startPage }" end="${endPage }">
			...
			<c:if test="${i != pageNum }">
				[ <a href="${conPath }/orderPastList.do?pageNum=${i}&mId=${mId}"> ${i } </a> ]
			</c:if>
		</c:forEach>
		<c:if test="${endPage<pageCnt }">
		  [ <a href="${conPath }/orderPastList.do?pageNum=${endPage+1}&mId=${mId}"> 다음 </a> ]
		</c:if>
	</div>
	</div>
